...
#ifdef __WINDOWS__
		dir.AppendDir("Restoria Map Editor");
#else
		dir.AppendDir(".rme");
#endif
		dir.AppendDir("data");
...
#  endif
	...
	if(title != "") {
		g_gui.root->SetTitle(title << " - Restoria Map Editor BETA" << TITLE_APPEND);
	} else {
		g_gui.root->SetTitle(wxString("Restoria Map Editor BETA") << TITLE_APPEND);
	}
#else
	if(title != "") {
		g_gui.root->SetTitle(title << " - Restoria Map Editor" << TITLE_APPEND);
	} else {
		g_gui.root->SetTitle(wxString("Restoria Map Editor") << TITLE_APPEND);
	}
#endif
}
